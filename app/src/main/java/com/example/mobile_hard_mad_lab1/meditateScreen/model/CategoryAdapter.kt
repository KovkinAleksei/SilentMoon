...
class CategoryAdapter(
    ...
    override fun onBindViewHolder(holder: CategoryViewHolder, position: Int) {
        ...
        categoryName.text = categoriesArray[position].name

        val categoryIcon = holder.itemView.findViewById<ImageView>(R.id.categoryIcon)
        categoryIcon.setImageResource(categoriesArray[position].icon)

        val categoryBg = holder.itemView.findViewById<View>(R.id.categoryBg)

        categoryBg.background.setTint(
            if (categoriesArray[position].isSelected)
                Color.parseColor("#8E97FD")
            else
                Color.parseColor("#A0A3B1")
        )
